...
Reglas:
1. Tu única fuente de verdad es el TEXTO OFICIAL incluido en este mensaje. Está prohibido completar, inferir o confirmar la respuesta usando memoria, conocimiento jurídico general o información externa.
2. Determina PRIMERO si ese texto, por sí solo, permite demostrar inequívocamente la respuesta. La RESPUESTA ALMACENADA EN BD sirve únicamente para comparar al final; nunca es evidencia.
3. Ten en cuenta si el enunciado pide CORRECTA o INCORRECTA y evalúa las cuatro opciones contra el texto.
4. Solo si el texto demuestra que una única opción es correcta, devuelve su letra.
5. Si para decidir necesitas otro artículo, otra disposición, una regla jurídica no reproducida en el texto, conocimiento externo o cualquier inferencia no demostrable directamente por el texto, debes devolver respuesta_auditor=null y resultado=NO_VERIFICABLE.
6. Si en tu fundamento afirmas o reconoces que el texto no basta, no contiene la información necesaria, no permite corroborar la opción o requiere contexto adicional, el resultado DEBE ser NO_VERIFICABLE y respuesta_auditor DEBE ser null.
7. Si hay ambigüedad, varias opciones defendibles, ninguna demostrable o el artículo aislado no basta: NO_VERIFICABLE.
8. Únicamente después de haber resuelto la pregunta con el texto: si la letra demostrada coincide con la almacenada, VALIDADA; si no coincide, CONFLICTO_RESPUESTA.
9. No intentes salvar una pregunta defectuosa ni marques VALIDADA porque la respuesta almacenada parezca jurídicamente plausible.
