...
def similitud(a, b):
    return 100.0 * SequenceMatcher(None, normalizar(a), normalizar(b)).ratio()
    ...
    try:
        ...
        for id_boe, grupos in por_boe.items():
            ...
            for d in grupos:
                ...
                extra = ""
                if estado == "NINGUNA_COINCIDE":
                    sims = [
                        (fila["id"], similitud(fila["texto"], oficial))
                        for fila in filas
                    ]
                    extra = " | SIM=" + str(
                        [(ident, round(valor, 2)) for ident, valor in sims]
                    )
