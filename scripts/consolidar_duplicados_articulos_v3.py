...
def main():
    ...
    try:
        ...
        if aplicar:
            ...
            # Cerrar la transacción de la simulación antes de exportarla.
            # v2 podía bloquearse al ejecutar backup() con esta transacción abierta.
            mem.commit()

            sello = datetime.now().strftime("%Y%m%d_%H%M%S")
            backup = DB.with_name(DB.stem + "_backup_antes_consolidar_" + sello + DB.suffix)
            temporal = DB.with_name(DB.stem + "_consolidada_temporal_" + sello + DB.suffix)

            origen.close()
            shutil.copy2(DB, backup)

            # Nunca escribir sobre la BD original durante la exportación.
            ...
            try:
                ...
                integrity = destino.execute("PRAGMA integrity_check").fetchone()[0]
                ...
                if integrity != "ok" or restantes_real != 0 or fk_real:
                    raise RuntimeError(
                        "Validación temporal falló: "
                        f"integrity={integrity}, duplicados={restantes_real}, FK={len(fk_real)}"
                    )
            except Exception:
                destino.close()
                if temporal.exists():
            ...
            # Sustitución final sólo después de validar completamente la copia consolidada.
            temporal.replace(DB)

            print(f"BACKUP.............................: {backup}")
            print("INTEGRITY FINAL....................: ok")
            print("DUPLICADOS FINALES................: 0")
            print("ERRORES FK FINALES................: 0")
