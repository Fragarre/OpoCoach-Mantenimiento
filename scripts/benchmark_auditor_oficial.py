...
BENCHMARK_IDS = [21, 45, 135, 183, 223, 527, 540, 618, 683, 900, 1087, 1095, 1368, 1522, 5198, 5230, 5373, 5385, 5388, 5520, 5577, 5751, 5753, 5775, 5814, 5815, 5868, 5913, 6093, 6182, 6184, 6229, 6236, 6249, 6256, 6280]
...
def auditar_api(db: Path, limite: int | None, fuente: str | None, modelo: str) -> Path:
    ...
    q = '''
        ...
        FROM lote_preguntas
        WHERE tipo_clasificacion='JURIDICA'
          AND id IN (21,45,135,183,223,527,540,618,683,900,1087,1095,1368,1522,5198,5230,5373,5385,5388,5520,5577,5751,5753,5775,5814,5815,5868,5913,6093,6182,6184,6229,6236,6249,6256,6280)
    '''
    params = []
    if fuente:
    ...
    out = REGISTROS / f'benchmark_auditor_oficial_{stamp}.csv'
...
'''
    ...
    con.close()
    print(f'Preguntas benchmark auditadas.......... {procesadas}')
...
def main():
    ...
    print()
    if not args.auditar:
        print(f'BENCHMARK: {len(BENCHMARK_IDS)} conflictos de fuentes oficiales apoyo/modelo.')
