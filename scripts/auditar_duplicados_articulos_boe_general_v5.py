...
def numero_desde_titulo(titulo):
    ...
    # Formatos numéricos del índice BOE:
    # "Artículo 10", "Artículo 10.", "Art. 10", "Art. 10."
    m = re.fullmatch(r"(?:Artículo|Art\.?)\s+(\d+)\.?", titulo, re.IGNORECASE)
    if m:
        return m.group(1)

    # Formatos escritos: "Artículo primero", "Artículo décimo", etc.
...
def main():
    ...
    try:
        ...
        print("=" * 100)
        print("AUDITORÍA GENERAL DE DUPLICADOS VS BOE VIGENTE — SOLO LECTURA")
        print("=" * 100)
        print(f"BD.....................: {DB}")
