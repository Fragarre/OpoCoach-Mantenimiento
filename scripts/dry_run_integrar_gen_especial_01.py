"""
Dry-run de integración del piloto GEN A1 / ESPECIAL 1.
...
- simula la sustitución del único marcador No determinado de ESPECIAL 1;
...
GEN_DEFECTO = CARPETA / "gen_revision" / "GEN-A1-ESPECIAL-01.json"
...
def leer_y_validar_gen(ruta: Path) -> dict:
    ...
    if not isinstance(articulos, list) or len(articulos) != 9:
        raise RuntimeError("El GEN debe contener exactamente 9 artículos.")
...
def sustituir_virtualmente(
    ...
    objetivo = [
        f
        for f in filas
        if f["parte"] == "ESPECIAL" and f["tema"] == "1"
    ]
    if len(objetivo) != 1 or not es_no_determinada(objetivo[0]):
        raise RuntimeError(
            "El CSV V1 debe contener exactamente una fila No determinada en ESPECIAL 1."
        )

    titulo = objetivo[0]["titulo"]
    nuevas = filas_especial_1(titulo, gen)
    ...
    if not sustituida:
        raise RuntimeError("No se pudo efectuar la sustitución virtual de ESPECIAL 1.")
    return salida
...
def aplicar_exclusividad(
    ...
    ordenadas = [
        ...
        for _, fila in sorted(
            enumerate(filas),
            key=lambda par: clave_orden(par[1], par[0]),
        )
    ]
...
def validar_sin_colisiones(filas: list[dict[str, str]]) -> None:
    ...
    for fila in filas:
        ...
        if anterior is not None and anterior != tema:
            raise RuntimeError(
                f"Colisión residual {fila['LEY']} art. {fila['articulo']}: "
                f"{anterior} vs {tema}"
            )
        vistos.setdefault(clave, tema)
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Simula, sin BD ni escritura de CSV, la integración GEN de A1 ESPECIAL 1."
    )
...
def main() -> int:
    ...
    es1 = [f for f in final if f["parte"] == "ESPECIAL" and f["tema"] == "1"]
    gen_es1 = [f for f in es1 if f["LEY"] == NOMBRE_GEN]
    directas_es1 = [f for f in es1 if f["LEY"] != NOMBRE_GEN]

    print("=" * 78)
    print("DRY-RUN INTEGRACION GEN A1 / ESPECIAL 1")
    print("=" * 78)
    ...
    print(f"Referencias directas ESPECIAL 1: {len(directas_es1)}")
    print(f"Artículos GEN ESPECIAL 1: {len(gen_es1)}")
    print(f"Referencias ESPECIAL 1 totales: {len(es1)}")
    print(f"No determinados después de resolver ESPECIAL 1: {len(nd_despues)}")
    ...
    if omitidas:
        ...
        for x in omitidas:
            print(
                ...
                f"-> ya asignado en {x['tema_prioritario']} "
                f"(raíz {x['articulo_raiz']})"
            )
    ...
    for art in gen["articulos"]:
        print(
            f"- art. {art['numero']} | {art['id_bloque']} | "
            f"sha256={art['hash_texto_final']}"
        )
