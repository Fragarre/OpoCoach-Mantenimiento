...
def ejecutar_capturando(cmd: list[str]) -> tuple[int, str]:
    print()
    print("=" * 78)
    print(" ".join(f'"{x}"' if " " in x else x for x in cmd))
    print("=" * 78)
    ...
    return int(proc.returncode), salida
...
def validar_documento_rag(db: Path, fuente: str) -> tuple[bool, str]:
    tipo = clasificar_fuente(fuente)

    if tipo != "BOE":
        rc = ejecutar(comando_proveedor(db, fuente))
        return rc == 0, f"proveedor={tipo}, rc={rc}"

    rc, salida = ejecutar_capturando(comando_proveedor(db, fuente))
    if rc == 0:
        return True, "BOE OK"

    requiere_fallback = (
        "SIN_INDICE_CONSOLIDADO" in salida
        or "requiere fallback PDF local" in salida
        or "REQUIERE_FALLBACK" in salida
    )
    if not requiere_fallback:
        return False, f"BOE rc={rc} sin fallback reconocido"

    script_pdf = SCRIPTS / "ampliar_corpus_pdf_local.py"
    if not script_pdf.is_file():
        return False, "BOE requiere fallback, pero falta ampliar_corpus_pdf_local.py"

    print(
        "\nProveedor BOE sin índice consolidado utilizable. "
        "Se prueba automáticamente el fallback PDF local."
    )
    rc_pdf = ejecutar(
        [
            sys.executable,
    ...
    if rc_pdf == 0:
        return True, "BOE sin índice + PDF local OK"

    return False, f"BOE sin índice + PDF local rc={rc_pdf}"
...
def main() -> int:
    ...
    for conv in convocatorias:
        ...
        for n, (fuente, norma) in enumerate(documentos, 1):
            ...
            ok_doc, detalle = validar_documento_rag(db, fuente)
            if not ok_doc:
                fallos_rag += 1
                print(f"RESULTADO DOCUMENTO: REQUIERE REVISIÓN ({detalle})")
            else:
                print(f"RESULTADO DOCUMENTO: VALIDACIÓN/PLAN OK ({detalle})")
