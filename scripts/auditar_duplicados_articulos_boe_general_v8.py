...
NUMEROS_ARTICULO = {
    ...
    # Ordinales compuestos usados, entre otras normas, por la LO 8/1980.
    "décimo": 10, "decimo": 10,
...
def obtener_articulos_eurlex(id_doue):
    ...
    resultado = {}
    # EUR-Lex no usa de forma uniforme la misma clase CSS para todas las
    # cabeceras de artículo. Identificamos la cabecera por su texto exacto.
    cabeceras = []
    for p in soup.find_all("p"):
        titulo = normalizar(p.get_text(" ", strip=True))
        m = re.fullmatch(r"Artículo\s+(\d+)(?:\s*bis)?\.?", titulo, re.IGNORECASE)
        if m:
            cabeceras.append((p, m.group(1)))

    for cabecera, numero in cabeceras:
        # El documento EUR-Lex incluye protocolos/anexos que vuelven a numerar
        # artículos desde 1. Conservamos la primera aparición con texto normativo.
        ...
        for elem in cabecera.find_all_next():
            if elem is cabecera:
                continue
            if elem.name == "p":
                texto_elem = normalizar(elem.get_text(" ", strip=True))
                if re.fullmatch(r"Artículo\s+\d+(?:\s*bis)?\.?", texto_elem, re.IGNORECASE):
                    break
            clases = elem.get("class", []) if hasattr(elem, "get") else []
            if elem.name == "p" and "norm" in clases:
